...
public class ObjectsTreeComposite
   ...
   */
  SkTreeViewerCheck treeViewerCheck;
   ...
   * Корневые узлы
   */
  private IListEdit<ITsNode> roots = new ElemArrayList<>();
  ...
  public ObjectsTreeComposite( Composite aParent, ITsGuiContext aContext, String aTitle ) {
    ...
    treeViewerCheck = new SkTreeViewerCheck( aContext ) {
      ...
      public void ensureGrayedState() {
        // не бывает тут серых - плоский список
      }
    ...
    SkTreeViewerCheckToolbar evTopPanel = new SkTreeViewerCheckToolbar( this, context, treeViewerCheck, aTitle );
  ...
  public void filterTreeModel( String aText2Filter ) {
    ...
    IListEdit<ITsNode> filteredRoots = new ElemArrayList<>();

    for( ITsNode rNode : roots ) {
      String name = rNode.name();
      if( search4.length() > 0 && !name.contains( search4 ) ) {
        continue;
      }
      filteredRoots.add( rNode );
    }
    treeViewerCheck.setRootNodes( filteredRoots );
